...
SELECT 
    ...
    , TO_CHAR(FROM_TZ(CAST(wo.closed_date AS TIMESTAMP), 'UTC') AT TIME ZONE 'Asia/Jakarta', 'YY/MM/DD HH24:MI') AS "Close date (System)"
    , TO_CHAR(FROM_TZ(CAST(wo.canceled_date AS TIMESTAMP), 'UTC') AT TIME ZONE 'Asia/Jakarta', 'YY/MM/DD HH24:MI') AS "Canceled date"
    , TO_CHAR(FROM_TZ(CAST(cd.first_close_date AS TIMESTAMP), 'UTC') AT TIME ZONE 'Asia/Jakarta', 'YY/MM/DD HH24:MI') AS "First Close Date"
    , TO_CHAR(FROM_TZ(CAST(cd.last_close_date AS TIMESTAMP), 'UTC') AT TIME ZONE 'Asia/Jakarta', 'YY/MM/DD HH24:MI') AS "Last Close Date"
...
WHERE 
    ...
    AND TRIM(UPPER(status_from.wo_status_name)) = 'CLOSED'
    AND TRIM(UPPER(status_to.wo_status_name)) = 'COMPLETED'
    AND cd.last_close_date IS NOT NULL
    AND TRUNC(CAST(wo.planned_start_date AS DATE), 'MM') != TRUNC(CAST(cd.last_close_date AS DATE), 'MM')
 __DYNAMIC_FILTERS__
ORDER BY 
